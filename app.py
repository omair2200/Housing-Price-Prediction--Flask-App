...
<head>
   ...
   <script>
    ...
    function updateCityOptions(callback = null) {
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        const filledCity = "{{ filled.get('city', '') }}";

        provinceSelect.addEventListener("change", () => updateCityOptions());

        // Call with callback to set selected city after city list is populated
        updateCityOptions(() => {
            if (filledCity) {
                const citySelect = document.querySelector('[name="city"]');
                Array.from(citySelect.options).forEach(opt => {
                    if (opt.value === filledCity) {
                        opt.selected = true;
                    }
                });
            }
                    ...
                    {% if field == 'availability_date' %}
                        <select name="{{ field }}" required>
                                {% for option in dropdowns[field] %}
                                    <option value="{{ option }}" {% if filled.get(field, 'immediate') == option %}selected{% endif %}>
                                        {{ option }}
                                    </option>
                                {% endfor %}
                        </select>                    
...
def home():
    ...
    if request.method == "POST":
        ...
        if 'availability_date' in df_input and '-' in str(df_input['availability_date'].values[0]):
            df_input['availability_date'] = 'immediate'
